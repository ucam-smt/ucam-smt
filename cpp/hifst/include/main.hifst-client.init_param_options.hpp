...
/** \file
 *  \brief To initialize boost parameter options for hifst-client
 * \date 8-8-2012
...
inline void init_param_options ( int argc, const char* argv[],
  ...
  try {
    ...
    //      ( HifstConstants::kNThreads.c_str(), po::value<uint>(), "Number of threads (trimmed to number of cpus in the machine) " )
    ( HifstConstants::kHifstHost.c_str(), po::value<std::string>(),
      "Server Host machine " )
    ( HifstConstants::kHifstPort.c_str(), po::value<std::string>(), "Server Port" )
    ( HifstConstants::kSourceLoad.c_str(), po::value<std::string>(),
      "Source text file " )
    ( HifstConstants::kTargetStore.c_str(), po::value<std::string>(),
      "Target text file " )
    ;
