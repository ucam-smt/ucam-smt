...
inline void init_param_options ( int argc, const char* argv[],
  ...
  try {
    ...
    ( HifstConstants::kWordPenaltyExtended.c_str(), po::value<std::string>(),
      "Range of word penalty values (i.e. 3.0 or 3.0:0.1:4.0 etc). Use %%wp%% in output to generate instances with different word penalties")
    ( HifstConstants::kNbestExtended.c_str(),
      po::value<uint>()->default_value (std::numeric_limits<uint>::max() ),
      "Apply Shortest path to obtain nshortest best hyps ")
    ( HifstConstants::kEpsilonLabels.c_str(),
      po::value<std::string>()->default_value ("0"),
      "Labels to be treated as epsilons")
    ( HifstConstants::kHifstSemiringExtended.c_str(),
      po::value<std::string>()->default_value (
        HifstConstants::kHifstSemiringStdArc.c_str() ),
      "Choose between stdarc and lexstdarc");
    ;
