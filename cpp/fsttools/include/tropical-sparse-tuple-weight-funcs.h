                                       ...
                                       unordered_set<uint> &idx )
{
  ...
  for ( StateIterator< VectorFst<TupleArc32> > si ( myfst ); !si.Done(); si.Next() ) {
    StateId state_id = si.Value();

    for ( MutableArcIterator< VectorFst<TupleArc32> > ai ( &myfst, si.Value() ); !ai.Done(); ai.Next() ) {
      const TupleW32 w = ai.Value().weight;

      for ( SparseTupleWeightIterator<FeatureWeight32, int> it ( w ); !it.Done(); it.Next() ) {
